...
- I segreti di sistema (JWT, ENCRYPTION_KEY) stanno in un `.env` sul server,
  **fuori
  dall'immagine**.
...
ENCRYPTION_KEY=<stringa casuale per cifrare le chiavi utente>
EOF
chmod 600 .env
```
Nota: qui NON c'è nessuna chiave Anthropic. In questo progetto la chiave la
mette ogni utente dall'app, e viene salvata cifrata nel database con
ENCRYPTION_KEY. Genera i tre segreti con:
```bash
openssl rand -hex 32
```
...
- La chat coach risponde (dopo che hai registrato un utente e inserito la sua
  chiave Claude dall'app).
